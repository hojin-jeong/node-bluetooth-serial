...
const DeviceINQ = require('./device-inquiry')
...
class BluetoothSerial extends EventEmitter {
    ...
    constructor() {
        ...
        this.$btBinding = require('bindings')('BluetoothSerialPort.node')
    ...
    listPairedDevices() {
        return new Promise(resolve => {
            this.$inq.listPairedDevices(pairedDevices => {
                resolve(pairedDevices)
            })
        })
    }
    inquire() {
        this.$inq.inquire(this.$_found, this.$_finish)
    }
    inquireSync() {
        this.$inq.inquireSync(this.$_found, this.$_finish)
    }
    findSerialPortChannel(address) {
        return new Promise((resolve, reject) => {
            this.$inq.findSerialPortChannel(address, channel => {
                if(channel >= 0) {
                    resolve(channel)
                } else {
                    reject()
    ...
    connect(address, channel) {
        return new Promise(async (resolve, reject) => {
            ...
            const connection = new this.$btBinding.BTSerialPortBinding(address, channel, onDeviceConnected.bind(this), onDeviceConnectFailed.bind(this))
        })
    }
    write(buffer) {
        return new Promise((resolve, reject) => {
...
class BluetoothSerialServer extends EventEmitter {
    ...
    constructor() {
        ...
        this.$btBinding = require('bindings')('BluetoothSerialPortServer.node')
        this.$constants = {
    ...
    listen(options) {
        return new Promise((resolve, reject) => {
            ...
            this.$server = new this.$btBinding.BTSerialPortBindingServer(onDeviceConnected.bind(this), onDeviceConnectFailed.bind(this))
        })
    }
    write(buffer) {
        return new Promise((resolve, reject) => {
