...
class BluetoothSerial extends EventEmitter {
    ...
    listPairedDevices() {
        return new Promise(resolve => {
            this.$btBinding.listPairedDevices(pairedDevices => {
                resolve(pairedDevices)
            })
        })
    }
    inquire() {
        this.$btBinding.inquire(this.$_found, this.$_finish)
    }
    inquireSync() {
        this.$btBinding.inquireSync(this.$_found, this.$_finish)
    }
    findSerialPortChannel(address) {
        return new Promise((resolve, reject) => {
            this.$btBinding.findSerialPortChannel(address, channel => {
                if(channel >= 0) {
                    resolve(channel)
                } else {
                    reject()
